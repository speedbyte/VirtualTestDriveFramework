...
namespace saliency_sandbox {
    namespace saliency {
        namespace activation {
            ...
            class _GBVS : public _Saliency<_width,_height,saliency_sandbox::utils::_HeatmapImage<_width,_height>> {
            private:
            ...
            public:
                ...
                void calc() override {
                    ...
                    n = this->properties()->template get<int>("iters",5);
                    sigma = this->properties()->template get<float>("sigma",4.0f);

                    // resize input to internal resolution
                    cv::resize(this->template input<0>()->value()->mat(),this->m_in_mat,cv::Size(_iwidth,_iheight),0.0f,0.0f,cv::INTER_AREA);
