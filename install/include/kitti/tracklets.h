...
//    namespace kitti {
        ...
        public:
            ...
            struct tTracklet {
                ...
                tTracklet(std::string objectType, float h, float w, float l, int first_frame, std::vector<tPose> poses,
                          int finished)
                        : objectType(objectType), h(h), w(w), l(l), first_frame(first_frame), poses(poses),
                          finished(finished) {}
